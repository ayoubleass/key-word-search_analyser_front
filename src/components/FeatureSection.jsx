    ...
    <section className={showProjectForm  || showForm ? css.concat('z-0') : css }>
      ...
      <div className="pmx-auto px-15 sm:px-6 lg:px-8 relative">
        <div className="flex  flex-wrap	 items-center justify-between	">
          ...
          {/*  */}
          <div className="w-full lg:w-1/2 ml-5 sm:mt-5 space-y-6">
            <div className="max-w-xl">
              <h2 className="text-2xl md:text-3xl font-bold text-[#2e326d] mb-10">
                Powerful SEO 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
          {[
            { title: 'Real-time Analysis', value: 'Instant keyword insights' },
            { title: 'Competition Metrics', value: 'Detailed competitor data' },
            { title: 'Trending Topics', value: 'Stay ahead of the curve' },
          ].map((stat, index) => (
            <div key={index} className="bg-white/50  backdrop-blur-sm rounded-lg p-4 shadow-md hover:shadow-lg transition-shadow duration-300 border border-gray-100">
              <h3 className="text-sm font-semibold text-[#2e326d]">{stat.title}</h3>
